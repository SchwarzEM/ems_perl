...
while (my $input = <$ID2>) {
    chomp $input;
    if ( $input =~ /\A (MGI:\d+) \t (\S+) \t [^\t]* \t [^\t]* \t [^\t]* \t [^\t]* \t (\S.*\S) \z/xms ) {
        # Sample input:
        # MGI:1891917     Ywhab   O       tyrosine 3-monooxygenase/tryptophan 5-monooxygenase activation protein, beta polypeptide        84.59   2       A2A5N1 A2A5N2 Q9CQV8
        my $mod_id       = $1;
        my $common_name  = $2;
        my $uniprots_txt = $3;
        my @uniprots = split /\s+/, $uniprots_txt;
        ...
        # Take advantage of these data to further map UniProts to MOD gene IDs.
