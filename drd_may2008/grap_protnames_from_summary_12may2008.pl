...
# Typical input line:
# Bacteria; Cyanobacteria; Chroococcales; Synechococcus; Synechococcus sp. CC9311 [Q0IA74_SYNS3 Q0I614_SYNS3]
...
while (my $input = <>) { 
    ...
    foreach my $pattern (@patts) { 
        if ($input =~ /$pattern/xms) { 
            # Any match? extract protnames once, and move on:
            if ($input =~ / \[ (\S.*\S) \] /xms) { 
                my $protline = $1;
                my @species_prots = split /\s/, $protline;
                foreach my $prot (@species_prots) { 
                    $total_prots{$prot} = 1;
                }
            }
            last SCAN;
...
foreach my $listed_prot (sort keys %total_prots) { 
    print "$listed_prot\n";
}
