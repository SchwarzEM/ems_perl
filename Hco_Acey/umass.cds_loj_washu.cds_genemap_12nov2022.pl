...
while (my $input = <$CDS_LOJ>) {
    ...
    if ( $input =~ /\A ID[=]cds[:] ([^\s;]+) .* \t .* gene= ([^\s;]+) .* \z/xms ) {
        ...
        # Reverse of gene mapping in washu.cds_loj_umass.cds_genemap_11nov2022.pl:
        # Acey v2 => WashU, rather than the other way around.

        if ( ( $gene1 =~ /\S/xms ) and ( $gene2 =~ /\S/xms ) ) {
            $data_ref->{'gene2'}->{ $gene2 }->{'gene1'}->{ $gene1 } = 1;
        }
    }
    elsif ( $input !~ /\A .+ \t [.] \z/xms) {
...
my @genes2 = grep { /\S/ } sort keys %{ $data_ref->{'gene2'} };
foreach my $gene2 (@genes2) {
    my @genes1 = grep { /\S/ } sort keys %{ $data_ref->{'gene2'}->{ $gene2 }->{'gene1'} };
    my $gene1_text = join '; ', @genes1;
