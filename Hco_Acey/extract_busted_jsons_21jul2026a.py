...
if len(sys.argv) < 2:
    ...
    print("Usage: extract_busted_jsons_21jul2026a.py <path_to_JSON_file_list.txt>", file=sys.stderr)
    sys.exit(1)

input_list_file = sys.argv[1]

header_text = "Pangene\tBranch\tLRT\tRate classes\tOmega_Distribution\tUncorrected_P-value\tCorrected_P-value"
...
with open(input_list_file, "r") as list_f:
    for line in list_f:
        ...
        try:
            ...
            # === 1. Find branches marked as "test" (same logic as aBSREL) ===
            ...
            # === Fallback for --branch All (exploratory mode) ===
                ...
                for bkey, battrs in ba_temp.items():
                    if isinstance(battrs, dict) and bkey != "attributes":
                        if (battrs.get("LRT") is not None or
                            ...
                            battrs.get("Corrected P-value") is not None or
                            battrs.get("Rate classes") is not None):
            ...
            # Get overall BUSTED test results (used for LRT and p-value)
            test_results = data.get("test results", {})
            overall_lrt = test_results.get("LRT", "None")
            overall_p   = test_results.get("p-value", "None")

            # === 3. Extract per test branch ===
            for branch_key, attrs in branch_attrs.items():
                ...
                # Rate classes and Omega distribution (per branch)
                ...
                # Print in the requested column order
                # Note: BUSTED provides one overall p-value (not per-branch corrected/uncorrected like aBSREL)
                print(pangene_name, display_name, overall_lrt, rate_classes, omega_str, overall_p, "N/A", sep="\t")
