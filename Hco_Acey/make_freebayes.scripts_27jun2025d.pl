...
foreach my $k (0..5) {
        ...
        my $chr     = $chrs[$k];
        my $short_chr = $chr;
        $short_chr =~ s/\ANecator_//;
        my $out_vcf = "Necator_2022.05.29.02.chrs.ngm.8_genos.$chr.naive.fb05.vcf";
        $out_vcf    = safename($out_vcf);

        $script = "job_Necator_freebayes.etc_2025.06.27.$j.sh";
        ...
        print $SCRIPT "freebayes -f $genome -r $chr";
        print $SCRIPT ' --haplotype-length 0 --min-alternate-count 1 --min-alternate-fraction 0 --pooled-continuous';
        print $SCRIPT " --report-monomorphic -g 200 --targets necator_aroian_2024.02.08.$short_chr.genes.cols1-3.bed";
