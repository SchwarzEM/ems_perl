...
while (my $input_genome = <>) {
    ...
    if ( $input_genome !~ /\A \S+ \.fa \z/xms ) {
        die "Can't parse input genome file name: $input_genome\n";
    }

    if ( $input_genome =~ /\A (\S+) \.fa \z/xms ) {
        $stem = $1;
        $stem =~ s/\.dna\.toplevel\z//;
        $stem =~ s/\.dna\.chromosome\.1\z//;
    }

    my $job1 = 'job_RScout_' . $stem . '_21jul2014a.sh';
    my $job2 = 'job_RScout_' . $stem . '_21jul2014b.sh';
    my $job3 = 'job_RScout_' . $stem . '_21jul2014c.sh';
    ...
    print $JOB1 '#PBS -l walltime=012:00:00', "\n";
    print $JOB1 '#PBS -l nodes=1:ppn=1', "\n";
    print $JOB1 '#PBS -l mem=50gb', "\n";
    ...
    print $JOB2 "RepeatScout -sequence $input_genome -freq $stem.freqs.out -output $stem.reps.raw.fa ;\n";
    ...
    print $JOB3 '#PBS -l walltime=012:00:00', "\n";
    print $JOB3 '#PBS -l nodes=1:ppn=1', "\n";
    print $JOB3 '#PBS -l mem=50gb', "\n";
    ...
    print $JOB3 "filter-stage-1.prl $stem.reps.raw.fa > $stem.reps.orig.fa ;\n";
