...
if ( $help or (! $infile) ) {
    die "Format: make_hbak_pe.salmon_2025.08.09.01.pl\n",
...
while (my $input = <$INFILE>) {
    ...
    if ( $input =~ /\A (\S+) \t (\S+) \z/xms ) { 
        ...
        # .R1.fq
        # /ocean/projects/mcb190015p/schwarze/heligmo/rnaseq/05_d.p.i_females/01/SRR15275138_1.fastq.gz
        ...
        my $output_script = "job_hbak_pe.salmon_2025.08.09.$j.sh";
        $output_script    = safename($output_script);

        my $output_prefix = $prefix . ".pe.salmon_2025.08.09.$j";
...
foreach my $prefix (@prefixes) {
    ...
    # Skip '--numBootstraps 100':
    print $OUT "salmon --no-version-check quant --threads 16 --seqBias --gcBias --posBias ";
    print $OUT '--index $PROJECT/heligmo/salmon/dbs/Hbak.nxHelBake1.1_gentrome_index ';
    print $OUT "--libType A ";
    print $OUT "--mates1 $input_data_1 ";
    print $OUT "--mates2 $input_data_2 ";
    print $OUT "--output $output_prefix ";
    print $OUT '--geneMap $PROJECT/heligmo/cds2gene_etc/hbakeri_stevens.cds2gene.tsv.txt ;';
