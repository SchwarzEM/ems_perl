...
while (my $infile_2 = <>) {
    ...
    # Sample input:
    # /ocean/projects/mcb190015p/schwarze/Acey/2019.05/pub_RNAseq_data_01/ACEY.L3i/L3i_SRR1124986_1.fastq.gz

    if ( $infile_2 =~ /\A (\S+) \/ [^\/\s]+ \/ (\S+) _2\.fastq\.gz  \z/xms ) {
        my $dir_stem  = $1;
        my $file_stem = $2;
        ...
        my $infile_1 = $infile_2;
        $infile_1    =~ s/_2\.fastq\.gz\z/_1.fastq.gz/;
	...
	die "Cannot parse input line: $infile_2\n";
    }
}
