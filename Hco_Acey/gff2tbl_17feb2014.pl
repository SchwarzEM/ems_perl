...
if ( $help 
    ...
    die "Format: gff2tbl_17feb2014.pl\n",
...
foreach my $source_seq (@source_seqs) {
    ...
    foreach my $gene_id (@genes) { 
        ...
        # However, in real life, this seems to produce massive failure in tbl2asn outputs; so, try giving NCBI only a little of what it wants, when it might actually make sense.

        # This part is nice and messy; but we can reasonably assume that we have complete boundaries for a gene if there are start or end codon nt at its furthest known coordinates.
        if ( ! exists $data_ref->{'source_seq'}->{$source_seq}->{'gene_id'}->{$gene_id}->{'start_codon_nt'}->{$gene_start_nt} ) { 
            $gene_start_nt = $ori2start_ineq{$gene_ori} . $gene_start_nt;
        }
        if ( ! exists $data_ref->{'source_seq'}->{$source_seq}->{'gene_id'}->{$gene_id}->{'stop_codon_nt'}->{$gene_end_nt} ) { 
