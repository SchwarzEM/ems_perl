...
while (my $input = <>) {
    ...
    if (! $header) { 
        ...
        if ( $input =~ / (?: [^\t]* \t){2} ([^\t]*) \t (?: [^\t]* \t){5} ([^\t]*) \t (?: [^\t]* \t){5} ([^\t]*) \t /xms ) { 
            ...
            #     have overrepresented GO terms for proteases
            ...
            if (     ( ( $input !~ /ENSG\d+\(human\)/xms ) and ( $input !~ /ENSMUSG\d+\(mouse\)/xms ) )
                 ...
                 and ( $phobius eq 'SigP' ) 
                 and ( $go_term =~ /(GO:0004190|GO:0004197|GO:0004222|GO:0004252|GO:0008236)/xms )    # for serine-type endopeptidase inhibitor activity == GO:0004867
               ) {
                print "$input\n";
            }
        }
