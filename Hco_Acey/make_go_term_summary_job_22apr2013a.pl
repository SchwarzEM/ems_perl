...
# Limit this version so that it only outputs A. ceylanicum.
my @data_sets = qw( acey_dirs );
...
foreach my $data_set (@data_sets) {
    foreach my $data_dir ( @{ $data_ref->{$data_set} } ) {
        ...
        if ( $dir_basename =~ /\A \S+ _RSEM \S+ \.log10\. (\S+) \.vs\. (\S+) \.func_wilcoxon_outdir \z/xms ) { 
            ...
            print "GO terms associated with upregulation from $denominator to $numerator in $data_synonym{$data_set}:\t\t\t\n";
            print "root_node_name\tnode_name\tnode_id\tp_high_ranks_after_refinement\n";
            print @upreg_term_text;
            print "\t\t\t\n";
            print "GO terms associated with downregulation from $denominator to $numerator in $data_synonym{$data_set}:\t\t\t\n";
