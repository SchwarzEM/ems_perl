...
# make_job_08sep2011.pl -- Erich Schwarz <emsch@caltech.edu>, 9/8/2011.
...
foreach my $single_infile (sort keys %single_inputs ) { 
    if ( $single_infile =~ /\A (\S+) \.single\.\S+ \z /xms ) { 
        $stem         = $1;
        $int_file_1   = $stem . '.single.08sep2011.filt1.fa';
        $out_unpaired = $stem . '.single.min35.noMOIdna2.08sep2011.fa';

        my $full_unpaired_name = 'first_reads.08sep2011/' . $single_infile;
        if (! -e $full_unpaired_name ) {   
            die "Can't find: $full_unpaired_name\n";
        }

        print '    filter_minimum_ACGT_fasta.pl -m 35 -i first_reads.08sep2011/',
              $single_infile,
              ' -o unpaired_reads.08sep2011/',
              $int_file_1,
              " ;\n",
              ;

        print '    bowtie ../sheep.aug2011/Ovis_Bos_BTdb -p 12 -t --un unpaired_reads.08sep2011/',
              $out_unpaired,
              ' -f unpaired_reads.08sep2011/',
              $int_file_1,
              " /dev/null ;\n",
              ;

        print '    rm unpaired_reads.08sep2011/',
              $int_file_1,
              " ;\n",
              ;
        print "\n";

        my $final_file = 'unpaired_reads.08sep2011/' . $out_unpaired;
...
foreach my $jumbled_infile (sort keys %jumbled_inputs ) { 
    if ( $jumbled_infile =~ /\A (\S+) \.jumbled\.\S+ \z /xms ) { 
        $stem         = $1;
        $int_file_1   = $stem . '.jumbled.08sep2011.filt1.fa';
        $int_file_2   = $stem . '.jumbled.08sep2011.filt2.fa';
        $out_paired   = $stem . '.paired.min35.noMOIdna2.08sep2011.fa';
        $out_unpaired = $stem . '.unpaired.min35.noMOIdna2.08sep2011.fa';

        my $full_jumbled_name = 'first_reads.08sep2011/' . $jumbled_infile;
        ...
        print '    filter_minimum_ACGT_fasta.pl -m 35 -i first_reads.08sep2011/',
              $jumbled_infile,
              ' -o paired_reads.08sep2011/',
              $int_file_1,
              " ;\n",
              ;

        print '    bowtie ../sheep.aug2011/Ovis_Bos_BTdb -p 12 -t --un paired_reads.08sep2011/',
              $int_file_2,
              ' -f paired_reads.08sep2011/',
              $int_file_1,
              " /dev/null ;\n",
              ;

        print '    paired_vs_unp_fastq.or.a.pl --fasta',
              ' -i paired_reads.08sep2011/',
              $int_file_2,
              ' -p paired_reads.08sep2011/',
              $out_paired,
              ' -u unpaired_reads.08sep2011/',
              $out_unpaired,
              " ;\n",
              ;

        print '    rm paired_reads.08sep2011/',
              $int_file_1,
              ' paired_reads.08sep2011/',
              $int_file_2,
              " ;\n",
              ;
        print "\n";

        my $final_file = 'paired_reads.08sep2011/' . $out_paired;
        if (exists $final_paired{$final_file}) {
            die "Redundant final paired file: $final_file\n";
        }
        $final_paired{$final_file} = 1;
        
        $final_file = 'unpaired_reads.08sep2011/' . $out_unpaired;
        if (exists $final_unpaired{$final_file}) {
            die "Redundant final unpaired file: $final_file\n";
        }
        $final_unpaired{$final_file} = 1;
...
foreach my $paired_infile (sort keys %paired_inputs ) { 
    if ( $paired_infile =~ /\A (\S+) \.paired\. (\S+) \z /xms ) { 
        ...
        $int_file_1   = $stem . '.jumbled.08sep2011.filt1.fa';
        $int_file_2   = $stem . '.jumbled.08sep2011.filt2.fa';
        $int_file_3   = $stem . '.jumbled.08sep2011.filt3.fa';
        $int_file_4   = $stem . '.jumbled.08sep2011.filt4.fa';
        $out_paired   = $stem . '.paired.min35.noMOIdna2.08sep2011.fa';
        $out_unpaired = $stem . '.unpaired.min35.noMOIdna2.08sep2011.fa';

        my $full_paired_name   = 'first_reads.08sep2011/' . $paired_infile;
        my $full_unpaired_name = 'first_reads.08sep2011/' . $unpaired;
        ...
        print '    cat paired_reads.08sep2011/',
              $int_file_1,
              ' paired_reads.08sep2011/',
              $int_file_2,
              ' > paired_reads.08sep2011/',
...
print "    program_done_e-ping.pl -p filtering_sorting_reads_08sep2011 ;\n";
print "\n";

my @unpaired_finals = sort keys %final_unpaired;
my @paired_finals   = sort keys %final_paired;

print "    velveth Hco_VelGen_09sep2011_allBGI_mild.khmer_min35_k35 35 \\ \n";
print "        -fasta \\ \n";
print "        -short \\ \n";
...
foreach my $vg_set (@velvetg_sets) { 
    ...
    foreach my $pai1 (@paired_finals) {
        if ( $pai1 =~ /\A paired_reads.08sep2011\/ (\S+) \.paired\. \S+ \z/xms ) { 
            $stem = $1;
...
print "    ; \n";
print "\n";
print "    program_done_e-ping.pl -p velveth_09sep2011_mild.khmer_allBGI_min35_k35 ; \n";
print "\n";
print "    velvetg Hco_VelGen_09sep2011_allBGI_mild.khmer_min35_k35 \\ \n";
...
print "\n";
print "    program_done_e-ping.pl -p velvet_assembly_09sep2011_mild.khmer_allBGI_min35_k35 ; \n";
print "\n";
print "    count_fasta_residues.pl Hco_VelGen_09sep2011_allBGI_mild.khmer_min35_k35/contigs.fa > full.count.09sep2011.txt ; \n";
print "    fasta_size_subset.pl -s --max 315M -i Hco_VelGen_09sep2011_allBGI_mild.khmer_min35_k35/contigs.fa | count_fasta_residues.pl > top.count.09sep2011.txt ; \n";
print "\n";
