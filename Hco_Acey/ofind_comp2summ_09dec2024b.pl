...
if ( (! $venom_list ) or (! $ofinds ) ) {
    die "Format: ofind_comp2summ_09dec2024b.pl [venom gene list] [OrthoFinder complete descs.] > [Sherm genes <tab> summaries with venoms noted]\n";
}
    ...
    if ( $input =~ /\A ( O\S*\d+\(\d+[ ]genes,\d+[ ]taxa\)[:]) \s+ (\S.+) \z /xms ) {
        ...
        while ( $gene_text =~ /(\S+\s*)/xmsg ) {
            ...
            if ( $gene =~ /\A (\S+) \( (\S+) \) \z/xms ) {
                my $gene_name = $1;
                my $taxon     = $2;
                if ( $taxon eq 's_hermaphroditum' ) {
                    push @sherms, $gene_name;
                }
        ...
        foreach my $sherm (@sherms) {
             print "$sherm\t$ogroup_id_text $desc_text\n";
        }
    }
    else {
