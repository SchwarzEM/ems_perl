...
if ( $help or (! $infile) ) {
    die "Format: make_acey_salmon_2019.05.28.01.pl\n",
...
while (my $input = <$INFILE>) {
    ...
    if ( $input =~ /\A (\S+) \t (\S+) \z/xms ) { 
        ...
        my $output_script = "job_acey_salmon_2019.05.28.$j.sh";
        $output_script    = safename($output_script);

        my $output_prefix = $prefix . ".salmon.2019.05.28.$j";
...
foreach my $prefix (@prefixes) {
    ...
    print $OUT '#SBATCH --ntasks-per-node=8', "\n";
    print $OUT '#SBATCH --constraint=EGRESS', "\n";
    print $OUT '#SBATCH --job-name=', $output_script, "\n";
    print $OUT '#SBATCH --mail-type=ALL', "\n";

    print $OUT "cd $curr_dir ;\n";

    print $OUT '. $SCRATCH/anaconda2/etc/profile.d/conda.sh ;', "\n";
    print $OUT 'conda activate salmon_0.13.1 ;', "\n";

    print $OUT "salmon --no-version-check quant --threads 8 --validateMappings ";
    print $OUT '--index $SCRATCH/Acey/2019.05/salmon/dbs/ancylostoma_ceylanicum.PRJNA231479.WBPS13.CDS_transcripts ';
    print $OUT "--libType A ";
    print $OUT "--unmatedReads $input_data ";
    print $OUT "--output $output_prefix ";
    print $OUT "--seqBias --gcBias ";
    print $OUT '--geneMap $SCRATCH/Acey/2019.05/salmon/annots/Acey_PRJNA231479.WBPS13.cds2gene.tsv.txt ';
    print $OUT "--numBootstraps 200 ;";
