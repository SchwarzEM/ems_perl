...
# UMass v2.1 E20201108-05 ES genes:
my $umass_v2_05 = q{};
...
# UMass v2.1 E20201108-07 ES genes:
my $umass_v2_07 = q{};
...
$washu_05    = $ARGV[2] if $ARGV[2];

$umass_v2_07 = $ARGV[3] if $ARGV[3];
$washu_07    = $ARGV[4] if $ARGV[4];

my $header = "Gene\tES_component\tES_observations\tWashU-spec_ES";

if (     ( ! -e $wa_g2um_g ) 
      or ( ! -e $umass_v2_05 ) or ( ! -e $washu_05 ) 
      or ( ! -e $umass_v2_07 ) or ( ! -e $washu_07 ) ) {
    die "Format: annot_es.1_02dec2022.pl ",
        "[WashU to UMass gene table] ",
        "[UMass v2.1 E-05 genes] [WashU E-05 genes] ",
        "[UMass v2.1 E-07 genes] [WashU E-07 genes] ",
        "> [output]",
        "\n";
}
...
while (my $input = <$UMASS_V2_05>) {
    ...
    if ( $input =~ /\A (\S+) \z/xms ) {
        ...
        die "From UMass v2.1 E20201108-05 input file $umass_v2_05, cannot parse: $input\n";
    }
}
close $UMASS_V2_05;
...
while (my $input = <$UMASS_V2_07>) {
    ...
    if ( $input =~ /\A (\S+) \z/xms ) {
        ...
        die "From UMass v2.1 E20201108-07 input file $umass_v2_07, cannot parse: $input\n";
    }
}
close $UMASS_V2_07;
