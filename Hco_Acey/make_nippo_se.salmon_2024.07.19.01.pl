...
if ( $help or (! $infile) ) {
    die "Format: make_nippo_se.salmon_2024.07.19.01.pl\n",
...
while (my $input = <$INFILE>) {
    ...
    if ( $input =~ /\A (\S+) \t (\S+) \z/xms ) { 
        ...
        # .R1.filt1.fq
        if ( $input_data_1 !~ / \S+ _trm \.(?:fastq|fq) (?:\.gz){0,1} \z/xms ) {
        ...
        my $output_script = "job_nippo_se.salmon_2024.07.19.$j.sh";
        $output_script    = safename($output_script);

        my $output_prefix = $prefix . ".se.salmon_2024.07.19.$j";
...
foreach my $prefix (@prefixes) {
    ...
    print $OUT "--libType A ";
    print $OUT "--unmatedReads $input_data_1 ";
