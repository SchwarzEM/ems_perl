    ...
    foreach my $gene_id (@genes) { 
        ...
        foreach my $tx_id (@txs) { 
            ...
            foreach my $feature (@features) { 
                my @start_nts = sort { $a <=> $b } 
                                keys %{ $data_ref->{'source_seq'}->{$source_seq}->{'gene_id'}->{$gene_id}->{'tx_id'}->{$tx_id}->{'feature'}->{$feature}->{'start_nt'} };
                foreach my $start_nt (@start_nts) { 
                    my $end_nt = $data_ref->{'source_seq'}->{$source_seq}->{'gene_id'}->{$gene_id}->{'tx_id'}->{$tx_id}->{'feature'}->{$feature}->{'start_nt'}->{$start_nt}->{'end_nt'};
                    my $ori    = $data_ref->{'source_seq'}->{$source_seq}->{'gene_id'}->{$gene_id}->{'tx_id'}->{$tx_id}->{'feature'}->{$feature}->{'start_nt'}->{$start_nt}->{'ori'};
                    my $phase  = $data_ref->{'source_seq'}->{$source_seq}->{'gene_id'}->{$gene_id}->{'tx_id'}->{$tx_id}->{'feature'}->{$feature}->{'start_nt'}->{$start_nt}->{'phase'};
                    print "$source_seq\t$gene_id\t$tx_id\t$feature\t$start_nt\t$end_nt\t$ori\t$phase\n";
                }
            }
        }
    }
