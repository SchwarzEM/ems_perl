...
# chemblprots2drugs_profuse.pl -- Erich Schwarz <emsch@its.caltech.edu>, 3/4/2011.
# Purpose: link data from four different MySQL outputs into a single *profuse* table linking ChEMBL protein names to intelligible drug targets.
...
if (    $opts{'help'} 
    ...
    die "Format: chemblprots2drugs.pl",
...
foreach my $prot_num1 ( @prot_nums1 ) { 
    ...
    foreach my $drug_num1 (@drug_numbers) {
        ...
        if ( $drug_common_name eq 'null' ) { 
            $drug_common_name = q{};
        }
        # Print an output line:
        print $prot_chembl_id,
              "\t",
              $drug_common_name,
              "\t",
              $drug_formula_namelist,
              "\n",
              ;
    }
}
