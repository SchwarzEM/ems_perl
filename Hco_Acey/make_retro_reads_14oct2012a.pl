...
while (my $input = <$INFILE>) { 
    ...
    if ( $input =~ /\A Lane [ ] [#] \d+ [ ] : [ ] \( (\d+) \) [ ] Index [ ] [#] (\d+) [ ] (\S.+\S) [ ] \( None \) \s* \z/xms ) { 
        ...
        print '    wget --user=gec --password=gecilluminadata',
              ...
              ' https://jumpgate.caltech.edu/runfolders/Volvox/120914_SN787_0129_AD1BAAACXX/Unaligned',
        ...
        print '    zcat ', $project, '*_R1_*.fastq.gz > /sternlab/redivivus/data02/schwarz/Acey_genomics/raw_', $raw_type, '_reads/', $project, '_', $desc, "_orig_14oct2012.R1.fq ;\n";

        # No need to do this for single-read data:
        # print '    zcat ', $project, '*_R2_*.fastq.gz > /sternlab/redivivus/data02/schwarz/Acey_genomics/raw_', $raw_type, '_reads/', $project, '_', $desc, "_orig_14oct2012.R2.fq ;\n";
        ...
        print '    retroname_fastq_reads.pl raw_', $raw_type, '_reads/', $project, '_', $desc, '_orig_14oct2012.R1.fq',
              ' > ', 
              'raw_', $raw_type, '_reads/', $project, '_', $desc, "_pre.retro_14oct2012.R1.fq ;\n";

        print '    rm raw_', $raw_type, '_reads/', $project, '_', $desc, "_orig_14oct2012.R1.fq ;\n";

        print '    quality_trim_fastq.pl -q 33 -u 50 -m 40 -n',
              ' -i raw_', $raw_type, '_reads/', $project, '_', $desc, '_pre.retro_14oct2012.R1.fq',
              ' -o raw_', $raw_type, '_reads/', $project, '_', $desc, "_retro_14oct2012.R1.fq ;\n",
              ;

        print '    rm raw_', $raw_type, '_reads/', $project, '_', $desc, "_pre.retro_14oct2012.R1.fq ;\n";

        print '    bowtie filter_seqs/CriGri1_AceyMito_IlluPrim -p 6 -t',
              ' --un filt2_', $raw_type, '_reads_23jul2012/', $project, '_', $desc, '_retro_14oct2012.no_cont.min40.R1.se.fq',
              ' -q raw_', $raw_type, '_reads/', $project, '_', $desc, '_retro_14oct2012.R1.fq',
              " /dev/null ;\n",
              ;

        print '    rm raw_', $raw_type, '_reads/', $project, '_', $desc, "_retro_14oct2012.R1.fq ;\n";
