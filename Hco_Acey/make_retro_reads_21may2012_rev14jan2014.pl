...
# make_retro_reads_21may2012_rev14jan2014.pl -- Erich Schwarz <ems394@cornell.edu>, 1/14/2014.
...
if ( (! $infile) or (! $raw_type) or (! $target_dir) ) { 
    die "Format: make_retro_reads_21may2012_rev14jan2014.pl\n",
...
while (my $input = <$INFILE>) { 
    ...
    if ( $input =~ /\A Lane [ ] [#] \d+ [ ] : [ ] \( (\d+) \) [ ] Index [ ] [#] (\d+) [ ] (\S.+\S) [ ] \( None \) \s* \z/xms ) { 
        ...
        print '    zcat ', $project, '*_R2_*.fastq.gz > ', $target_dir, '/raw_', $raw_type, '_reads/', $project, '_', $desc, "_orig_2014.01.14.R2.fq ;\n";
