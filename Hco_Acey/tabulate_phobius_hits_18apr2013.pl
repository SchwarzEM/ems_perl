...
# tabulate_phobius_hits_18apr2013.pl -- Erich Schwarz <ems394@cornell.edu>, 4/18/2013.
# Purpose: legacy for previous version of 'tabulate_phobius_hits.pl'.
...
foreach my $gene1 (@genes) { 
    ...
    foreach my $tx1 (@txs) {
        if ( $data_ref->{'gene'}->{$gene1}->{'tx'}->{$tx1}->{'phob_annot'} !~ /\A\s*\z/xms ) { 
            push @phob_annots, $data_ref->{'gene'}->{$gene1}->{'tx'}->{$tx1}->{'phob_annot'}; 
        }
    }
    if (@phob_annots) { 
        $phob_annot_text = join '; ', @phob_annots;
