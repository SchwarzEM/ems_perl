...
if ( $help or (! $gene2cds) or (! $annot) ) { 
    die "Format: cds2gene_EnTAP_annot.pl\n",
        "        --gene2cds|-g  [gene-to-CDS table]\n",
        "        --annot|-a     [EnTAP GO term cds-annots in TSV]\n",
...
while (my $input = <$ANNOT>) {
    ...
    if ( $input !~ /\A [#] /xms ) { 
        if ( $input =~ /\A (\S+) \t (.+) \z/xms ) { 
            ...
            if (! $no_split ) {
                while ( $orig_annots =~ /(GO[:]\d+-.*?\(L=\d+\))/xmsg ) {
                    my $annot = $1;
                    if ( $annot =~ /[;]/xms ) {
                        die "Internal semicolon in annotation: $annot\n",
                            "From input line: $input\n",
                            ; 
                    }
