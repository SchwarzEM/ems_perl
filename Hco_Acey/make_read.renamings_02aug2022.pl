...
while (my $input = <>) {
    ...
    if ( $input =~ /\A (N\d+) \/ (.+ R1 .+ \.fq) \z/xms ) {
        ...
        print $JOB1 'PATH="$PROJECT/ems_perl/Hco_Acey:$PATH" ;', "\n";
        print $JOB1 "cat $infile_1 | reject_junk_rnaseq_read-1s_02aug2022.pl > $outfile_1 ;\n";
        print $JOB1 "cat $infile_2 | reject_junk_rnaseq_read-2s_02aug2022.pl > $outfile_2 ;\n";
