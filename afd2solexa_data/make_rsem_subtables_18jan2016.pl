...
while (my $infile = <>) {
    chomp $infile;
    # Sample input line:
    # RSEM_results_2016.01.17/rsem_atml1_3_rep1_2016.01.17.01.genes.results
    if ( $infile =~ /\A RSEM_results_2016\.01\.17\/rsem_ (\S+) _2016\.01\.17\.\d{2}\.genes\.results \z/xms) {
        my $orig_tag = $1;
        my $tag      = $orig_tag;
        ...
        $outfile     =~ s/RSEM_results_2016.01.17/RSEM_results_2016.01.17_subtables/;
