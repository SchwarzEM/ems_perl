...
while (my $input = <>) {
    ...
    # 
    # desired output line:
    # /mnt/ls15/scratch/users/emsch/work_rsync/2015/adrienne/rsem_2016/replicate_3nt.exact_1/atml1_3_rep1.trim_exact_3nt.fq	144	60
    ...
    if ( $input =~ /\A (\S+) (\t \d+ \t \d+) \z/xms ) {
        ...
        if ( $file =~ /rsem_2016\/replicate_(\d+)\/\S+\.fq\z/xms ) {
            my $rep = $1;
            $file =~ s/rsem_2016\/replicate_$rep\//rsem_2016\/replicate_3nt.exact_$rep\//;
            $file =~ s/\.fq\z/\.trim_exact_3nt\.fq/;
            if (! -e $file) {
