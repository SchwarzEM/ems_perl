...
# wbgenify_cdsed_file.pl -- Erich Schwarz <emsch@its.caltech.edu>, 1/9/2008.
...
my $wormpep_archival = $ARGV[0];
my $wormpep_recent   = $ARGV[1];
my $cds_linestarts   = $ARGV[2];
...
open (my $CDS_LINESTARTS, "<", "$cds_linestarts") 
    or die "Can't open $cds_linestarts: $!";
while (my $input = <$CDS_LINESTARTS>) {
    chomp $input;
    if ( $input =~ /\A (\S+) (\s.*) /xms ) { 
        my $orig_cds  = $1;
        my $remainder = $2;
