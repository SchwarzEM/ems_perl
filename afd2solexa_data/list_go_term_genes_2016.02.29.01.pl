                    ...
                    if ( $cond_text ne 'not sig. gene' ) { 
                        ...
                        foreach my $condition (@conditions) {
                            ...
                            if (! exists $data_ref->{'condition'}->{$condition}->{'tair_gene'} ) {
                                die "Cannot recognize nominal condition \"$condition\" in: $input\n";
                            }
...
my @go_terms = sort keys %{ $data_ref->{'go_term'} };
foreach my $go_term (@go_terms) {
    my @all_genes      = sort keys %{ $data_ref->{'go_term'}->{$go_term}->{'total_annot'}->{'gene'} };
    my $all_gene_count = @all_genes;

    my @conditions     = sort keys %{ $data_ref->{'go_term'}->{$go_term}->{'condition'} };
    my @cond_all_genes = ();
    my @cond_go_genes  = ();
    ...
    print "$go_term\n";
    print "Number of all GO term-annotated genes:         $go_annot_gene_no\n";
    print "Number of all genes annot. with \"$go_term\":  $all_gene_no [$all_annot_ratio]\n";
    print "Number of genes sig. assoc. w/ genot. change:  $cond_all_gene_no\n";
    print "No. genes annot. w/ \"$go_term\" + gen. chg.:  $cond_go_gene_no [$sig_annot_ratio]\n";

    print "List of genes annot. w/ $go_term and sig. assoc. w/ genotypic-change: $cond_go_gene_text\n";
    print "\n";
}
