...
while (my $infile = <>) {
    ...
    # or:                       !                                           !
    # RSEM_3nt_results_2016.01.24/rsem_atml1_3_rep1.trim_exact_3nt_2016.01.23.01.genes.results

    if ( $infile =~ /\A RSEM_3nt_results_2016\.01\.2\d\/rsem_ (\S+) \.trim_exact_3nt_2016\.01\.2\d\.\d{2}\.genes\.results \z/xms) {
        my $orig_tag = $1;
        my $tag      = $orig_tag;
        ...
        $outfile     =~ s/RSEM_3nt_results_2016.01.\d{2}/RSEM_3nt_results_2016.01.24_subtables/;
