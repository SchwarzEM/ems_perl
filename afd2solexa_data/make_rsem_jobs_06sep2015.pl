...
while (my $input = <>) {
    ...
    if ( $input =~ /\A (\S+) \s+ (\d+) \s+ (\d+) \z/xms ) {
        ...
        my $job      = 'job_rsem_sepals_' . $read_tag . "_2015.09.06.$j";
        my $job_file = "$job.sh";
        ...
        print $JOB '#PBS -r n', "\n";
        print $JOB '#PBS -V', "\n";
        print $JOB 'cd /mnt/lustre_scratch_2012/schwarz/work/2015/adrienne/rsems_2015.09.06 ;', "\n";
        ...
        print $JOB " $reads";
        print $JOB ' /mnt/lustre_scratch_2012/schwarz/work/2015/adrienne/rsem_2015/indices/Athal_sepal_seqs_2015.07.26.rsem';
