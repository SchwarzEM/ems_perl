...
foreach my $infile (@input_files) { 
    ...
    while (my $input = <$INPUT_FILE>) { 
        chomp $input;
        # sample input line:
        # molecular_function      nucleic acid binding    GO:0003676      +       0.997011        0.00298991      0.992429        0.00757156 
        # molecular_function      high affinity zinc uptake transmembrane transporter activity    GO:0000006      -       -1      -1      -1      -1
        if ( $input =~ / \A 
