...
# single_LC_table_13jul2012.pl -- Erich Schwarz <emsch@its.caltech.edu>, 7/13/2012.
...
# Revised so that 0/0 RPKM is always *1*, not '0' except when I explicitly compute its logarithm...
...
#   WBGene00009119|F25H2.5	0.818	98437.54	0.00
        ...
        if ( $input =~ / \A (\S+) \t [^\t]* \t (\d+\.\d+) \t (\d.\d{2}) \s* \z /xms) { 
            $gene      = $1;
            $rpkm      = $2;
            $mult_frac = $3;
...
foreach my $imp_gene (sort keys %important_gene) { 
    ...
    # For some programs, it is easiest to feed them absolute numbers and let the program do the log-tranform.
    # However, these programs are likely to Tragically Fail if told to take the log of 0.00.
    # Thus, for this reason alone, an cosmetically acceptable variant 'absolute' measurement is also created, 
    #     with a minimum value of 0.01.
    foreach my $expr_type3 (sort keys %infiles) { 

        # First, make sure there is *some* value here, before setting it to 0.01!
        $data_ref->{$imp_gene}->{$expr_type3}->{'RPKM_no_zeros'} = 
            $data_ref->{$imp_gene}->{$expr_type3}->{'RPKM'};

        if ( $data_ref->{$imp_gene}->{$expr_type3}->{'RPKM'} == 0.00 ) {
            $data_ref->{$imp_gene}->{$expr_type3}->{'RPKM_no_zeros'} = 0.01;
        }

        # Again, give a default value:
        $data_ref->{$imp_gene}->{$expr_type3}->{'RPKM_non_multi_no_zeros'} = 
            $data_ref->{$imp_gene}->{$expr_type3}->{'RPKM_non_multi'};

        if ( $data_ref->{$imp_gene}->{$expr_type3}->{'RPKM_non_multi'} == 0.00 ) {
            $data_ref->{$imp_gene}->{$expr_type3}->{'RPKM_non_multi_no_zeros'} = 0.01;
        }
    }
    ...
    my $L3_rpkm_nz       = $data_ref->{$imp_gene}->{'L3'}->{'RPKM_no_zeros'};
    my $L4_rpkm_nz       = $data_ref->{$imp_gene}->{'L4'}->{'RPKM_no_zeros'};
    my $nhr67_rpkm_nz    = $data_ref->{$imp_gene}->{'nhr'}->{'RPKM_no_zeros'};
    my $N2_rpkm_nz       = $data_ref->{$imp_gene}->{'N2'}->{'RPKM_no_zeros'};
    
    my $L3_rpkm_nm_nz    = $data_ref->{$imp_gene}->{'L3'}->{'RPKM_non_multi_no_zeros'};
    my $L4_rpkm_nm_nz    = $data_ref->{$imp_gene}->{'L4'}->{'RPKM_non_multi_no_zeros'};
    my $nhr67_rpkm_nm_nz = $data_ref->{$imp_gene}->{'nhr'}->{'RPKM_non_multi_no_zeros'};
    my $N2_rpkm_nm_nz    = $data_ref->{$imp_gene}->{'N2'}->{'RPKM_non_multi_no_zeros'};
    ...
    my $max_imp_nm_N2_ratio           = safely_divide_RPKMs($max_imp_rpkm_nm, $N2_rpkm_nm, 0.01);
    my $max_imp_nm_N2_ratio_log10     = safely_divide_RPKMs($max_imp_rpkm_nm, $N2_rpkm_nm, 0.01, 'log', 10);
    ...
    my $max_wtLC_nm_N2_ratio       = safely_divide_RPKMs($max_wtLC_rpkm_nm, $N2_rpkm_nm, 0.01); 
    my $max_wtLC_nm_N2_ratio_log10 = safely_divide_RPKMs($max_wtLC_rpkm_nm, $N2_rpkm_nm, 0.01, 'log', 10); 
    ...
    # Get the [wt L3]/[nhr-67(-) L4] ratio data, with simple and non-multi. RPKMs:
    my $L3_nhr67_ratio          = safely_divide_RPKMs($L3_rpkm,    $nhr67_rpkm,    0.01);
    my $L3_nhr67_ratio_log10    = safely_divide_RPKMs($L3_rpkm,    $nhr67_rpkm,    0.01, 'log', 10);
    my $L3_nhr67_nm_ratio       = safely_divide_RPKMs($L3_rpkm_nm, $nhr67_rpkm_nm, 0.01);
    my $L3_nhr67_nm_ratio_log10 = safely_divide_RPKMs($L3_rpkm_nm, $nhr67_rpkm_nm, 0.01, 'log', 10);
    ...
    # Get the [wt L4]/[nhr-67(-) L4] ratio data, with simple and non-multi. RPKMs:
    my $L4_nhr67_ratio          = safely_divide_RPKMs($L4_rpkm,    $nhr67_rpkm,    0.01);
    my $L4_nhr67_ratio_log10    = safely_divide_RPKMs($L4_rpkm,    $nhr67_rpkm,    0.01, 'log', 10);
    my $L4_nhr67_nm_ratio       = safely_divide_RPKMs($L4_rpkm_nm, $nhr67_rpkm_nm, 0.01);
    my $L4_nhr67_nm_ratio_log10 = safely_divide_RPKMs($L4_rpkm_nm, $nhr67_rpkm_nm, 0.01, 'log', 10);

    # Get the [wt L3]/[wt L4] ratio data, with simple and non-multi. RPKMs:
    my $L3_L4_ratio          = safely_divide_RPKMs($L3_rpkm,    $L4_rpkm,    0.01);
    my $L3_L4_ratio_log10    = safely_divide_RPKMs($L3_rpkm,    $L4_rpkm,    0.01, 'log', 10);
    my $L3_L4_nm_ratio       = safely_divide_RPKMs($L3_rpkm_nm, $L4_rpkm_nm, 0.01);
    my $L3_L4_nm_ratio_log10 = safely_divide_RPKMs($L3_rpkm_nm, $L4_rpkm_nm, 0.01, 'log', 10);

    # For each individual LC type, get the <LC type>/[N2] ratio data, in human-readable plain and log[10] form:

    my $L3_N2_ratio          = safely_divide_RPKMs($L3_rpkm,    $N2_rpkm,    0.01);
    my $L3_N2_ratio_log10    = safely_divide_RPKMs($L3_rpkm,    $N2_rpkm,    0.01, 'log', 10);
    my $L3_N2_nm_ratio       = safely_divide_RPKMs($L3_rpkm_nm, $N2_rpkm_nm, 0.01);
    my $L3_N2_nm_ratio_log10 = safely_divide_RPKMs($L3_rpkm_nm, $N2_rpkm_nm, 0.01, 'log', 10);

    my $L4_N2_ratio          = safely_divide_RPKMs($L4_rpkm,    $N2_rpkm,    0.01);
    my $L4_N2_ratio_log10    = safely_divide_RPKMs($L4_rpkm,    $N2_rpkm,    0.01, 'log', 10);
    my $L4_N2_nm_ratio       = safely_divide_RPKMs($L4_rpkm_nm, $N2_rpkm_nm, 0.01);
    my $L4_N2_nm_ratio_log10 = safely_divide_RPKMs($L4_rpkm_nm, $N2_rpkm_nm, 0.01, 'log', 10);

    my $nhr67_N2_ratio          = safely_divide_RPKMs($nhr67_rpkm,    $N2_rpkm,    0.01);
    my $nhr67_N2_ratio_log10    = safely_divide_RPKMs($nhr67_rpkm,    $N2_rpkm,    0.01, 'log', 10);
    my $nhr67_N2_nm_ratio       = safely_divide_RPKMs($nhr67_rpkm_nm, $N2_rpkm_nm, 0.01);
    my $nhr67_N2_nm_ratio_log10 = safely_divide_RPKMs($nhr67_rpkm_nm, $N2_rpkm_nm, 0.01, 'log', 10);
    ...
    if (! $no_rounding) {
        # Basic data:
        # No need to round $L3_rpkm, $L4_rpkm, $nhr67_rpkm, $N2_rpkm, or their *_nz derivatives -- they're prerounded to 0.01!
        ...
        $L3_rpkm_nm_nz    = sprintf "%.2f", $L3_rpkm_nm_nz;
        $L4_rpkm_nm_nz    = sprintf "%.2f", $L4_rpkm_nm_nz;
        $nhr67_rpkm_nm_nz = sprintf "%.2f", $nhr67_rpkm_nm_nz;
        $N2_rpkm_nm_nz    = sprintf "%.2f", $N2_rpkm_nm_nz;
        ...
        $L3_nhr67_ratio          = sprintf "%.2f", $L3_nhr67_ratio;
        $L3_nhr67_ratio_log10    = sprintf "%.3f", $L3_nhr67_ratio_log10;
        $L3_nhr67_nm_ratio       = sprintf "%.2f", $L3_nhr67_nm_ratio;
        $L3_nhr67_nm_ratio_log10 = sprintf "%.3f", $L3_nhr67_nm_ratio_log10;
        ...
        $L4_nhr67_ratio          = sprintf "%.2f", $L4_nhr67_ratio;
        $L4_nhr67_ratio_log10    = sprintf "%.3f", $L4_nhr67_ratio_log10;
        $L4_nhr67_nm_ratio       = sprintf "%.2f", $L4_nhr67_nm_ratio;
        $L4_nhr67_nm_ratio_log10 = sprintf "%.3f", $L4_nhr67_nm_ratio_log10;

        $L3_L4_ratio             = sprintf "%.2f", $L3_L4_ratio;
        $L3_L4_ratio_log10       = sprintf "%.3f", $L3_L4_ratio_log10;
        $L3_L4_nm_ratio          = sprintf "%.2f", $L3_L4_nm_ratio;
        $L3_L4_nm_ratio_log10    = sprintf "%.3f", $L3_L4_nm_ratio_log10;

        $L3_N2_ratio             = sprintf "%.2f", $L3_N2_ratio;
        $L3_N2_ratio_log10       = sprintf "%.3f", $L3_N2_ratio_log10;
        $L3_N2_nm_ratio          = sprintf "%.2f", $L3_N2_nm_ratio;
        $L3_N2_nm_ratio_log10    = sprintf "%.3f", $L3_N2_nm_ratio_log10;

        $L4_N2_ratio             = sprintf "%.2f", $L4_N2_ratio;
        $L4_N2_ratio_log10       = sprintf "%.3f", $L4_N2_ratio_log10;
        $L4_N2_nm_ratio          = sprintf "%.2f", $L4_N2_nm_ratio;
        $L4_N2_nm_ratio_log10    = sprintf "%.3f", $L4_N2_nm_ratio_log10;

        $nhr67_N2_ratio          = sprintf "%.2f", $nhr67_N2_ratio;
        $nhr67_N2_ratio_log10    = sprintf "%.3f", $nhr67_N2_ratio_log10;
        $nhr67_N2_nm_ratio       = sprintf "%.2f", $nhr67_N2_nm_ratio;
        $nhr67_N2_nm_ratio_log10 = sprintf "%.3f", $nhr67_N2_nm_ratio_log10;
    }
    ...
    my @header_columns = qw( Gene
                             ...
                             L3_nz         L4_nz          nhr-67_nz         N2_nz
                             L3_nm_nz      L4_nm_nz       nhr-67_nm_nz      N2_nm_nz
                             ...
                             L3/nhr        log10(L3/nhr)  L3_nm/nhr_nm      log10(L3_nm/nhr_nm)
                             L4/nhr        log10(L4/nhr)  L4_nm/nhr_nm      log10(L4_nm/nhr_nm)
                             L3/L4         log10(L3/L4)   L3_nm/L4_nm       log10(L3_nm/L4_nm)

                             L3/N2         log10(L3/N2)   L3_nm/N2_nm       log10(L3_nm/N2_nm)
                             L4/N2         log10(L4/N2)   L4_nm/N2_nm       log10(L4_nm/N2_nm)
                             nhr/N2        log10(nhr/N2)  nhr_nm/N2_nm      log10(nhr_nm/N2_nm)
    );
    ...
    my @data_to_print = ( $L3_rpkm,     $L4_rpkm,     $nhr67_rpkm,    $N2_rpkm,     $max_wtLC_rpkm,     $max_wtLC_N2_ratio,
                          ...
                          $L3_rpkm_nz,        $L4_rpkm_nz,           $nhr67_rpkm_nz,     $N2_rpkm_nz,
                          $L3_rpkm_nm_nz,     $L4_rpkm_nm_nz,        $nhr67_rpkm_nm_nz,  $N2_rpkm_nm_nz,
                          ...
                          $L3_nhr67_ratio,    $L3_nhr67_ratio_log10, $L3_nhr67_nm_ratio, $L3_nhr67_nm_ratio_log10,
                          $L4_nhr67_ratio,    $L4_nhr67_ratio_log10, $L4_nhr67_nm_ratio, $L4_nhr67_nm_ratio_log10,
                          $L3_L4_ratio,       $L3_L4_ratio_log10,    $L3_L4_nm_ratio,    $L3_L4_nm_ratio_log10,

                          $L3_N2_ratio,       $L3_N2_ratio_log10,    $L3_N2_nm_ratio,    $L3_N2_nm_ratio_log10,
                          $L4_N2_ratio,       $L4_N2_ratio_log10,    $L4_N2_nm_ratio,    $L4_N2_nm_ratio_log10,
                          $nhr67_N2_ratio,    $nhr67_N2_ratio_log10, $nhr67_N2_nm_ratio, $nhr67_N2_nm_ratio_log10,
    );
...
sub safely_divide_RPKMs { 
    ...
    # No logarithms of zero -- ever! a previous version mistakenly made this conditional on $_take_log:
    if ( $_numerator == 0 ) {
