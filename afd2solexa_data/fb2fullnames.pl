...
# fb2fullnames.pl  -- Erich Schwarz <ems394@cornell.edu>, 3/21/2020.
# Purpose: given a nametable for FBgn\d+ genes, throughout input text(s), change "FBgn\d+" to "FBgn\d+|name", then print results to STDOUT.
...
if ($help or (! $names) or (! @infiles) ) { 
    die 'Format: fb2fullnames.pl  --names|-n [file format: (FBgn\d+ \s+ pubname )] --input|-i [1+ files or \'-\' stream] --help|-h [print this message]', "\n", ;
}
