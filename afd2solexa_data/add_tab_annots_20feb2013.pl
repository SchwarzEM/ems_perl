...
# add_tab_annots.pl -- Erich Schwarz <ems394@cornell.edu>, 2/13/2013.
# Purpose: given index genefile and 1+ nonredund. annot. TSVs, append annots to index text by "\t" or "; ".  Note that this is better engineered for tabbed input than semicolon input; it will enforce uniform tabcounts for each input file, then reliably fills in tabs for genes that do not have annotations.  This needs to also be implemented for semicolon input, but is considerably less important, so gets short shrift.
    ...
    if ( exists $index_lines_ref->{'i'}->{$line_no} ) { 
        ...
        if (! $semicolon) { 
            ...
            foreach my $append_file (@annot_files) { 
                ...
                if ( (! $semicolon) and (! exists $index_lines_ref->{'gene'}->{$annot_gene}->{'annot'}->{$append_file} ) ) { 
