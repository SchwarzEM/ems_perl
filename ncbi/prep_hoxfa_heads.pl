...
# prep_hoxfa_heads.pl -- Erich Schwarz <emsch@its.caltech.edu>, 9/22/2008.
# Purpose: given homology texts and Hox paper sequence names, prepare headers for Sequin.
        ...
        while ( my $input = <$FASTA> ) { 
            ...
            if ( $input =~ /\A > (\S+) /xms ) { 
                ...
                if ($fa_id =~ /\A Cbre_ /xms ) {
                    ...
                    $strain  = '[strain=CB5161]';
                }
                if ($fa_id =~ /\A Csp3_ /xms ) { 
                    $species = '[organism=Caenorhabditis sp. PS1010]';
                    $strain  = '[strain=PS1010]';
                }
                # Add 'lcl|' prefix for NCBI -- which may, or may not, actually be helpful.
                print ">lcl|$fa_id  $species  $strain";
