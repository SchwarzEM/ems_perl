...
if ( $help 
    ...
    die "Format: gff2tbl_11oct2017.pl\n",
        ...
        "    --type|-t      [GFF type: currently, only 'augustus|aug', (~equivalently)'gtf2', 'gff3', or 'agat']\n",
...
foreach my $gff_file (@gff_files) { 
    ...
    while (my $input = <$GFF_FILE>) { 
        ...
        if ( $input !~ /\A [#]/xms ) { 
            ...
            # Sample input for the overall span and orientation of a given gene, for AUGUSTUS GFF3 or AGAT-generated GFF3::
            ...
            # Sherm_X AUGUSTUS        gene    25369   26967   .       +       .       ID=Sherm_X.g16155

            elsif ( ( ( $ok_type{$gff_type} eq 'gff3') or ( $ok_type{$gff_type} eq 'agat') )
            ...
            # Parse AUGUSTUS GTF2 or AUGUSTUS GFF3 gene body data:
            ...
            elsif ( ( ( $ok_type{$gff_type} eq 'aug') or ( $ok_type{$gff_type} eq 'gff3') or ( $ok_type{$gff_type} eq 'agat') ) 
                    and 
                    ( $input =~ /\A (\S+)           # $source_seq
                                    \t [^\t]* \t 
                ...
                if ( ( $ok_type{$gff_type} eq 'gff3') or ( $ok_type{$gff_type} eq 'agat') ) {
