#!/usr/bin/perl
# go2citace.pl
# Erich Schwarz <emsch@its.caltech.edu>, 2004/12/01
# Derived from go2ace.pl, v. 1.1, 2001/09/13, L. Stein.

# Purpose: send .ace format to STDOUT for GO_term objects, with -D objects for citace
# Note: go2citace.pl requires a directory containing following files:
...
for my $gofile (@ontologies) {
  ...
  while(<ONT>){
    ...
    my $record = qq(-D GO_term : "GO:$4"\n\n);       # to wipe old records in citace
    $record .= qq(GO_term : "GO:$4"\n);   # start catenating with ".="
