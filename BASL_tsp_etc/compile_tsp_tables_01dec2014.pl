...
foreach my $new2orig_name (@new2orig_names) {
    ...
    while (my $input = <$ORIG>) {
        ...
        if ( $input =~ /\A > /xms ) { 
            if ( $input =~ /\A > ( \S+ _ [A-Z]+ _ \d+ [-] \d+) \s+ ([a-z]+ \| ([^\|\s]+) \| .*) \z/xms ) {
                my $seqname   = $1;
                my $orig_info = $2;
                my $uniprot   = $3;
                ...
                $data_ref->{'seqname'}->{$seqname}->{'orig_info'} = $orig_info;
            }
            else {
    ...
    while (my $input = <$FASTA>) {
        ...
        if ( $input =~ /\A > /xms ) {
            if ( $input =~ /\A > ( \S+ (_ [A-Z]+ _ \d+ [-] \d+)) (.*) \z/xms ) {
                ...
                if ( (! exists $data_ref->{'seqname'}->{$seqname}->{'uniprot'} ) or (! exists $data_ref->{'seqname'}->{$seqname}->{'orig_info'} ) ) {
                    die "Cannot map original sequence name $seqname to UniProt ID or original sequence information\n";
                }
                else {
                    my $uniprot   = $data_ref->{'seqname'}->{$seqname}->{'uniprot'};
                    my $orig_info = $data_ref->{'seqname'}->{$seqname}->{'orig_info'};
                    my $new_seqname = $seqname;  # by default
                    if ( exists $data_ref->{'uniprot'}->{$uniprot}->{'gene'} ) {
                        my $gene = $data_ref->{'uniprot'}->{$uniprot}->{'gene'};
                        $new_seqname = $gene . $seqsuffix;
                    }
                    print "$new_seqname\t$orig_info\n";
                }
            }
            else {
                die "In FASTA file $fasta, cannot parse header line: $input\n";
            }
        }
    }   
    close $FASTA;
