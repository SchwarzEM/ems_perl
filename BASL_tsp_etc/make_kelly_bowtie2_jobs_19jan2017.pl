...
while (my $input = <>) {
    chomp $input;
    if ( $input =~ /\A \S+ \/ \S+ _ ([^_\s]+) _ [ACGT]+ _ R1\.filt1\.fq \z/xms ) { 
        my $allele   = $1;
        my $out_pref = "$allele.bowtie2.WBcel235";

        my $job1 = 'job_bowtie2_' . $allele . '_mapping_2017.01.19.01.sh';
        my $job2 = 'job_samtools_' . $allele . '_wrangling_2017.01.19.01.sh';
        ...
        print $JOB1 '#PBS -r n', "\n";
        print $JOB1 '#PBS -V', "\n";
        print $JOB1 "cd /mnt/scratch/emsch/nre-1_mut_seqs ;\n";
        print $JOB1 "module load bowtie2/2.2.6 ;\n";
        print $JOB1 "module load SAMTools/1.2 ;\n";
        print $JOB1 "bowtie2 -p 8 --qc-filter --no-unal -x bt2_dbs/WBcel235.index -U $input -S $out_pref.sam ;\n";
        ...
        print $JOB2 '#PBS -r n', "\n";
        print $JOB2 '#PBS -V', "\n";
        print $JOB2 "cd /mnt/scratch/emsch/nre-1_mut_seqs ;\n";
