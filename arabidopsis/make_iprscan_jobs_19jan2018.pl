...
$workdir  = $ARGV[3] if $ARGV[3];
$job_stem = $ARGV[4] if $ARGV[4];

if ( ( $stem !~ /\A \S+ \z/xms ) 
     or (! looks_like_number($limit) ) 
     or ( $limit < 1 ) 
     or ( $limit != int($limit) ) 
     or (! -d $workdir ) 
     or ( $job_stem !~ /\A \S+ \z/xms ) 
   ) {
    die "make_iprscan_jobs_19jan2018.pl\n",
        "    [stem -- must be single block of text]\n",
        "    [limit -- must be positive integer]\n",
        "    [optional censored qsub jobs, listed as X,Y,Z -- joined with commas]\n",
        "    [working directory in which jobs will be set and run]\n",
        "    [job name stem, e.g., \"job_iprscan_nigoni_2016.12.01\"]\n",
        ;
}
...
foreach my $i (1..$limit) {
    ...
    my $job1 = "$job_stem.$file_no.sh";
    my $job2 = "$job_stem.$next_file_no.sh";
