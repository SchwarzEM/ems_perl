...
# replace_clone_names_in_tfa_headers.pl -- Erich Schwarz <emsch@its.caltech.edu>, 12/17/2007.
# Purpose: Go through a FASTA file and, in its headers, replace old with new + space-old clone names.
...
while (my $input = <>) { 
    foreach my $old_name (@old_names) { 
        if ( $input =~ /\A > $old_name/xms ) { 
            $input =~ s/$old_name/$renamed{$old_name}  $old_name/;
        }
    }
    print $input;
}
