...
# pfam_tbl2roster.pl -- Erich Schwarz <emsch@its.caltech.edu>, 2/2/2010.
# Purpose: given Pfam-A.hmm, cds2gene_Nspp.tsv, and PROTEOME.hmmscan.Pfam-A.tbl, make descending genes/PFAM roster.
# NOTE: recent format change in table output of HMMER 3.0 forced a script change, and in fact may have made it significantly obsolete.
...
# February 2010 sample input:
# 
#    #                                                               --- full sequence ----
#    # target name        accession  query name           accession    E-value  score  bias
#    #------------------- ---------- -------------------- ---------- --------- ------ -----
#    RhoGAP               PF00620.20 2RSSE.1              -            5.8e-23   80.8   0.0
#    Astacin              PF01400.17 4R79.1a              -            7.5e-69  230.6   0.3

    if ( ( $input !~ /\A [#] /xms ) 

         # Nov. 2009 rule:
         # and ( $input =~ /\A (\S+) \s+ (\S+) \s+ \S /xms ) ) {
         # -- should recode to make optional, or better yet, cut out steps!

         # For now, just recode to use newer HMMER table output:

         and ( $input =~ /\A (\S+) \s+ \S+ \s+ (\S+) \s /xms ) ) { 

        # Capture data:
        $name = $1;
        $prot = $2;

        # Strictly enforce reality of data:
