...
#
# Minor edits by Erich Schwarz <emsch@its.caltech.edu>, 6/8/2009.
...
while (my $line = <GFF>) {
  ...
  # Added to cope gracefully with MtDNA lines, which won't need migration:
  if ( $line =~ / \A MtDNA \s /xms ) { 
      print OUT $line,"\n";
      next;
  }
...
sub remap_gff {
  ...
  if ($chromosome !~ /^(I|II|III|IV|V|X)$/) {
    die "Unrecognised chromosome ID: '$chromosome'\nshouldThe chromosome ID must be one of I, II, III, IV, V, X\n";
  }
