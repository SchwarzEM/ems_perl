...
# taxid_children.pl -- Sujai Kumar <sujai.kumar@ed.ac.uk> -- 7/10/2010.
...
open NODES, "<$nodefile"
  or die "Could not open nodes.dmp at the location specified";
...
while ( my $line = <NODES> ) {
    ...
    next
      if $1 == $2
    ; # if any node has itself as a parent (eg taxid 1) then the recursion doesn't stop and the script falls over
    push @{ $childof{$2} }, $1;
}
