...
# blast_separate_taxa.pl -- Sujai Kumar <sujai.kumar@ed.ac.uk> -- 7/10/2010.
...
die
"Usage: blast_separate_taxa.pl -b1 <blast1file> -b2 <blast2file> [-d <min bitscore difference, default 50>]"
...
while (<$b1fh>) {
    chomp;
    my @hsp = split /\t/;
    next unless scalar @hsp == 12;    #12 fields in a blast tabular hit
    ...
    $b1hash{ $hsp[0] } = $hsp[11]
      unless exists $b1hash{ $hsp[0] }
          and $b1hash{ $hsp[0] } > $hsp[11];    #store best bit score
}
close $b1fh;
my %b2hash;
...
while (<$b2fh>) {
    ...
    next if defined $perc_id  and $hsp[2] < $perc_id;
    $b2hash{ $hsp[0] } = $hsp[11]
      unless exists $b2hash{ $hsp[0] }
          and $b2hash{ $hsp[0] } > $hsp[11];    #store best bit score
}
close $b2fh;
...
for my $b1key ( keys %b1hash ) {
    if ( exists $b2hash{$b1key} ) {
        ...
        delete $b2hash{$b1key
          }; # b2hash{b1key} removed so that it isnt printed when printing b2 only
    }
    else {
        print B1ONLY "$b1key\t$b1hash{$b1key}\n";
    }
