...
while (my $input = <@ARGV>) {
    ...
    # Allow a genome sequence to be supplied, ONCE; require it be a full filename.
    ...
    elsif ( $input =~ /\A ( \/mnt\/ls15\/scratch\/users\/emsch\/work_rsync\/2015\/caenogens\/ \S+ \/ [^\/\s]+ _s1_p0\.)
                     (\d)
                     \.bax\.h5 
                     \z /xms ) {
        my $stem = $1;
        ...
        if ( $j == 1 ) {
            ...
            if ( ( exists $seen{$stem} ) and ( exists $seen{1} ) and ( exists $seen{2} ) ) {
                ...
                my $fofn_file = "tropicalis.$suffix.bax.h5_filelist_2016.07.31.$file_no.fofn";
                $fofn_file    = safename($fofn_file);

                my $cmp_h5 = "tropicalis.$suffix.bax.h5_filelist_2016.07.31.$file_no.cmp.h5";
                $cmp_h5    = safename($cmp_h5);

                my $progress = "tropicalis.$suffix.bax.h5_filelist_2016.07.31.$file_no.pbalign_progress.txt";
                $progress    = safename($progress);

                my $start_time = "tropicalis.$suffix.bax.h5_filelist_2016.07.31.$file_no.start_time.txt";
                ...
                my $qsub_file = "job_tropicalis.$suffix.pbalign_2016.07.31.$file_no.sh";
                $qsub_file    = safename($qsub_file);

                my $next_qsub_file = "job_tropicalis.$suffix.pbalign_2016.07.31.$next_file_no.sh";
