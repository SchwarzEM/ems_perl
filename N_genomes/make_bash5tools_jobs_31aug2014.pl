...
my $qsub_start_script = 'run_bash5tools_jobs_31aug2014.sh';
...
while (my $input = <>) { 
    ...
    if ( $input =~ /\A ( \/mnt\/home\/emsch\/work\/Ngen\/pacbio\/meyer\/ [^\s\/]+ \/ [^\s\/]+ \/ Analysis_Results ) \/ ( ( [^\s\/]+ ) \.bas\.h5 ) \z/xms ) {
        ...
        $i++;
        my $j = sprintf "%02i", $i;
        my $job_name = "job_bash5tools_31aug2014.$j";
        ...
        print $JOB "bash5tools.py --outFilePrefix $prefix.subreads --readType subreads --outType fasta --minLength 100 $input_bas_h5_file ;\n";
