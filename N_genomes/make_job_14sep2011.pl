...
my $older_date       = '13sep2011';
my $newer_date       = '14sep2011';

my $in_reads_dir     = 'khmerized_reads.' . $older_date;
...
my %abnormal_stem = ( '10513_30DY0AAXX_c151_l1_r12'  => 1,
                      '10513_ilmn200901_c202_l1_r12' => 1,
                      '11079_42FC3AAXX_c152_l7_r12'  => 1,
                      '11080_42FC3AAXX_c152_l8_r12'  => 1, );

my $fix_abn_pair_num = q{ --r1 } . q{'} . q{/} . q{1} . q{'} . q{ --r2 } . q{'} . q{/} . q{2} . q{' };
...
foreach my $single_infile (sort keys %single_inputs ) { 
    if ( $single_infile =~ /\A (\S+) \.single\.\S+ \z /xms ) { 
        $stem         = $1;
        $out_unpaired = $stem . '.single.min35.filt_sodd.' . $newer_date . '.fa';
...
foreach my $jumbled_infile (sort keys %jumbled_inputs ) { 
    if ( $jumbled_infile =~ /\A (\S+) \.jumbled\.\S+ \z /xms ) { 
        ...
        $out_paired   = $stem . '.paired.min35.filt_sodd.' . $newer_date . '.fa';
        $out_unpaired = $stem . '.unpaired.min35.filt_sodd.' . $newer_date . '.fa';
        ...
        print $fix_abn_pair_num if $abnormal_stem{$stem};
        print ' -i ', $out_paired_dir, q{/}, 
              $int_file_1,
              ' -p ', $out_paired_dir, q{/}, 
              $out_paired,
...
my $velvet_assembly_dir = 'velveth PS1010_VelGen_' . $newer_date . '_k35';

print "    $velvet_assembly_dir 35 \\ \n";
print "        -fasta \\ \n";
print "        -short \\ \n";
...
print "    program_done_e-ping.pl -p velveth_", $newer_date, "_PS1010_min35_k35 ; \n";
print "\n";

print "    velvetg $velvet_assembly_dir \\ \n";
...
print "    program_done_e-ping.pl -p velvet_assembly_", $newer_date, "_PS1010_min35_k35 ; \n";
print "\n";

print "    count_fasta_residues.pl $velvet_assembly_dir",
      '/contigs.fa > full.count.',
      $newer_date, 
      ".txt ; \n",
      ;
