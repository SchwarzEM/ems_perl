...
while (my $input = <>) {
    ...
    # E.g.:
    # /mnt/ls15/scratch/users/emsch/work_rsync/2015/caenogens/nigoni/quiver/bax_h5/m150725_222852_42137_c100833552550000001823181811251532_s1_p0.3.bax.h5

    if ( $input =~ /\A (\/mnt\/ls15\/scratch\/users\/emsch\/work_rsync
        ...
        if ( $j == 1 ) {
            ...
            if ( ( exists $seen{$stem} ) and ( exists $seen{1} ) and ( exists $seen{2} ) ) {
                ...
                my $fofn_file = "nigoni_bax.h5_filelist_2015.11.12.$file_no.fofn";
                $fofn_file    = safename($fofn_file);

                my $cmp_h5 = "nigoni_bax.h5_filelist_2015.11.12.$file_no.cmp.h5";
                $cmp_h5    = safename($cmp_h5);

                my $progress = "nigoni_bax.h5_filelist_2015.11.12.$file_no.pbalign_progress.txt";
                $progress    = safename($progress);

                my $start_time = "nigoni_bax.h5_filelist_2015.11.12.$file_no.start_time.txt";
                ...
                my $qsub_file = "job_nigoni_mhap_haploid_pbalign_2015.11.12.$file_no.sh";
                $qsub_file    = safename($qsub_file);

                my $next_qsub_file = "job_nigoni_mhap_haploid_pbalign_2015.11.12.$next_file_no.sh";
                ...
                print $QSUB '#PBS -r n', "\n";
                print $QSUB '#PBS -V', "\n";
                print $QSUB 'cd /mnt/ls15/scratch/users/emsch/work_rsync/2015/caenogens/nigoni/quiver ;', "\n";
                print $QSUB "echo `date` > $start_time ;\n";
                print $QSUB "$smrtwrap $pbalign --verbose --nproc 8",
                            " --forQuiver $fofn_file",
                            " nigoni_mhap_pbjelly.decont_2015.11.11.fasta",
                            " $cmp_h5",
                            ' 2>',
                            "$progress",
                            " ;\n",
