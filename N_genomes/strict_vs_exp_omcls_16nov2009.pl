...
# strict_vs_exp_omcls_16nov2009.pl -- Erich Schwarz <emsch@its.caltech.edu>, 10/16/2009.
...
if ( ($help) or (! -r $input_omcl ) ) { 
    die "Format: prot2gene_omcl.pl --input_omcl|-i",
        " [orthomcl.out] --expandable|-e [expandable taxa]\n",
        ;
}
    ...
    if ($input =~ / \A
        ...
        foreach my $o_prot (@orthoprots) { 
            ...
            if ( $o_prot =~ / \A [^\s\(\)]+ \( ( [^\s\(\)]+ ) \) \z /xms ) { 
                ...
                if ( (! exists $exp_taxa{$species} ) and ( exists $seen_taxon{$species} ) ) { 
                    $printable = 0;
                }
