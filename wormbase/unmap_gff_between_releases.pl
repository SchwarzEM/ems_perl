...
#   unmap_gff_between_releases.pl                 
# 
# by Gary Williams                         
#
# This unmaps the clone positions and genomic sequences between two
# releases i.e. it remaps positions going backwards in time
#
# Last updated by: $Author: gw3 $     
# Last updated on: $Date: 2006/06/06 09:29:56 $      

# Source, 10/23/2012:  ftp://ftp.sanger.ac.uk/pub2/wormbase/software/Remap-between-versions/remap.tar.bz2
# Syntax:              unmap_gff_between_releases.pl -gff test.gff -out test.out -release1 150 -release2 160
...
if ($release1 > $release2) {
  die "release $release1 is larger than release $release2 - you must to have these the other way around\n";
}
...
while (my $line = <GFF>) {
  ...
  my @f = split /\t/, $line;
      ...
      ($f[3], $f[4], $f[6], $indel, $change) = unmap_gff($chromosome, $start, $end, $sense, $release2, $release1, @mapping_data);
...
sub unmap_gff {
  ...
  if ($chromosome =~ /chr([IVX]+)/i) {$chromosome = $1;}
  if ($chromosome eq 'MtDNA') {return ($start, $end, $sense, 0, 0);} # the mitochondrion is invariant
  if ($chromosome !~ /^(I|II|III|IV|V|X)$/) {
    die "Unrecognised chromosome ID: '$chromosome'\nThe chromosome ID must be one of I, II, III, IV, V, X\n";
  }
                                                                                                                                         
  # NB we stop one release before the target lowest release number
  # because the data is for the mapping from one release to another and the 
  # data for 'release X' is really for the mapping between releases 'X-1' and 'X'
  for (my $release = $release2; $release > $release1; $release--) { # NB we stop one release before the target release number
...
=head2 NAME - unmap_gff_between_releases.pl

=head1 USAGE

=over 4

=item unmap_gff_between_releases.pl [options]

=back

This script reads in a GFF file and the numbers of two releases of
wormbase and maps the chromosomal locations of the latest release 
...
=item -release1 The first (earlier) database version to convert back to e.g. 140

=back

=item -release2 The second (later) database version to convert from e.g. 150
