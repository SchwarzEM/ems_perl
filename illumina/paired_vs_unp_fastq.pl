...
# paired_vs_unp_fastq.pl -- Erich Schwarz <emsch@its.caltech.edu>, 11/3/2010.
# Purpose:  Take four lines at a time.  Check for superficially correct headers.  Namesort (avoiding RAM and brute force) into paired and unpaired output files.
    ...
    if ( $read_name =~ /\A(\S+)(?:$opts{'r1'}|$opts{'r2'})\z/ ) {
        ...
        if ( exists $data_ref->{'singleton'}->{$read_stem} ) { 
            ...
            if ( $head1 =~ /\A[@](\S+)$opts{'r1'}\z/ ) {
                ...
                die "Unable to resolve which paired-end a jumbled read was.\n";
            }
        }
        else { 
            die "Unable to resolve status of read.\n";
        }
    }
    else { 
        die "Unable to complete parsing text data from input file $opts{'input'}.\n";
    }
}
close $INFILE,  or die "Cannot open close filehandle to jumbled-reads file $opts{'input'}: $!";
