...
# paired_vs_unp_fastq.or.a.pl -- Erich Schwarz <emsch@its.caltech.edu>, 2/28/2011.
# Purpose:  Take two or four lines at a time, depending on FASTA or (default) FASTQ input.  Check for superficially correct headers.  Namesort (avoiding RAM and brute force) into paired and unpaired output files.  Cope with the recent decision by Illumina to code '1' and '2' ends with a BLANK SPACE between their markers and the header name.
...
# Default suffixes for paired-end read 1 and paired-end read 2 -- new-style Illumna headers, not pre-11/2011 ones!
$opts{'r1'} = '[ ]1';
$opts{'r2'} = '[ ]2';

# Note that the former defaults were:
# $opts{'r1'} = '#0\/1';
# $opts{'r2'} = '#0\/2';
# 
# These have been relegated to optional, specified status because they are MUCH easier to type 
# into a command line than '[ ]1' and '[ ]2' -- actually, I don't currently know how to do the latter,  
# except by using Unicode strings (ugh), or by falling back on the workably but sloppy solution '\\s1' and '\\s2'.
...
if ( $opts{'help'} ) {
    ...
    print "       --r1                  <suffix1>  suffix marking paired-end read 1, default \"", '[ ]1', "\"; type \"", '#0\/1', "\" for older-style end.\n";
    print "       --r2                  <suffix2>  suffix marking paired-end read 2, default \"", '[ ]2', "\"; type \"", '#0\/2', "\" for older-style end.\n";
...
# The program silently refuses to overwrite existing files for paired or unpaired outputs:
if (! $opts{'paired'} ) { 
...
while (my $input = <$INFILE>) {
    ...
    if ($j == 1) { 
        ...
        # Here and below, avoid normal 'xms' parsing to prevent ambiguous regexes.

        # Enforce presence of the lead pattern in the first (read name) header of the stanza:
        if ( $input !~ /\A$lead_pattern\S+/ ) { 
            die "Can't parse putative header: $input\n";
        }

        # Note that, to cope with Illumina's latest brainwave, this version of the parser allows blank spaces 
        #     to become part of $read_name, *if* they are part of the obligatory suffixes $opts{'r1'} or +$opts{'r2'} -- 
        #     although the rest of $read_name must still be all non-spaces.
        my $read_name = q{};
        if ( ( $input =~ /\A$lead_pattern(\S+$opts{'r1'})/ ) or ( $input =~ /\A$lead_pattern(\S+$opts{'r2'})/ ) ) {
            $read_name = $1;
        }
        ...
        if ( ( $read_name !~ /\A\S+$opts{'r1'}\z/ ) and ( $read_name !~ /\A\S+$opts{'r2'}\z/ ) ) { 
            ...
            if ( $read_name =~ /\A(\S+)$opts{'r1'}\z/ ) {
                ...
                if ( exists $data_ref->{'observed_name'}->{$read_stem}->{'r1'} ) { 
                    ...
                    # Otherwise, it is jumbled, not singleton or pairable.
                    else { 
                        $data_ref->{'jumbled'}->{$read_stem} = 1;
                        delete $data_ref->{'singleton'}->{$read_stem} ;
                        delete $data_ref->{'maybe_paired_properly'};
...
while (<$INFILE>) { 
    ...
    my $read_name = q{};
    if ( $head1 =~ /\A$lead_pattern(\S+(?:$opts{'r1'}|$opts{'r2'}))/ ) { 
        $read_name = $1;
    }
    if ( $head1 !~ /\A$lead_pattern(\S+(?:$opts{'r1'}|$opts{'r2'}))/ ) {
        die "Can't read name from first header line in FASTQ/A stanza: $head1\n";
    }
    if ( $read_name =~ /\A(\S+)(?:$opts{'r1'}|$opts{'r2'})\z/ ) {
        ...
        if ( exists $data_ref->{'singleton'}->{$read_stem} ) { 
            ...
            if ( $head1 =~ /\A$lead_pattern(\S+)$opts{'r1'}/ ) {
                $data_ref->{'data'}->{$read_stem}->{'r1'} = $read_data_ref;
            }
            elsif ( $head1 =~ /\A$lead_pattern(\S+)$opts{'r2'}/ ) {
