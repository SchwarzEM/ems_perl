...
# split_fasta.or.q_reads.pl -- Erich Schwarz <emsch@caltech.edu>, 10/3/2012.
# Purpose: given an integer N and a FASTA/Q file of Illumina reads (not mainline sequences!), make N subfiles with ~equal numbers of entries; makes huge jumbled FASTA/Q readfiles easier to sort by divide-and-conquer.
...
sub die_loudly {
    die "Format: split_fasta.or.q_reads.pl\n",
        "    -f [FASTA input, default; readfile, not other kind of sequence file]\n",
        "    -q [FASTQ input; readfile only; mutually exclusive with -f]\n",
